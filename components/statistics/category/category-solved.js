...
/**
 * 카테고리별 문제 해결 현황을 보여주는 컴포넌트
 * @param {Object} data - 카테고리별 통계 데이터 (mockStatisticsResponseData.js - mockCategoryStatisticsData 참고)
 */
export default function CategorySolvedChart({ data }) {
  const [chartType, setChartType] = useState("bar");

  // 차트를 그리기 위해 label, value, colors, maxValue 데이터를 추출
  const { labels, values, maxValue } = useMemo(
    () =>
      statisticUtils.responseTransformers.categoryResponseDto.toCategorySolvedChartData(
        data
      ),
    [data]
  );
  ...
  const renderChart = () => {
    switch (chartType) {
      ...
      case "radar":
        return <RadarChart data={processedData} maxValue={maxValue} />;
      default:
        return null;
    }
  };
      ...
      >
        ...
        <FormControl size="small" sx={{ minWidth: 120 }}>
          ...
          >
            ...
            <MenuItem value="pie">원형 그래프</MenuItem>
